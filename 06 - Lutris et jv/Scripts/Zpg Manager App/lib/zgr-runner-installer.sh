...
# Détermination du dossier de recherche initial
search_dir="."
if [ -n "$1" ] && [ -f "$1" ]; then
...
# ---------------------------------------------------------------------------------------------
# MODE ARGUMENT UNIQUE STRICT (si un fichier spécifique est passé en argument)
if [ -n "$1" ] && [ ! -f "$1" ]; then
  target_arg_runner="$1"
  
  declare -A temp_file_or_url
  declare -A temp_source_type
  
  shopt -s nullglob
  for file in "$search_dir"/*.zgr; do
    filename=$(basename "$file")
    r_name="${filename%.zgr}"
    if [ "$r_name" = "$target_arg_runner" ]; then
      temp_source_type["$target_arg_runner"]="[Local]"
      temp_file_or_url["$target_arg_runner"]="$file"
      break
    fi
  done

  if [ -z "${temp_file_or_url[$target_arg_runner]}" ] && (command -v curl >/dev/null 2>&1 || command -v wget >/dev/null 2>&1); then
...
' "$target_arg_runner")
      if [ -n "$dl_url" ]; then
        temp_source_type["$target_arg_runner"]="[En ligne]"
        temp_file_or_url["$target_arg_runner"]="$dl_url"
      fi
    fi
  fi

  if [ -z "${temp_file_or_url[$target_arg_runner]}" ]; then
    zenity --error --text="Le runner '$target_arg_runner' est introuvable." 2>/dev/null
    exit 1
  fi

  if [ -d "$lutris_runner_dir/$target_arg_runner" ]; then
    zenity --info --title="Déjà installé" --text="Le runner '$target_arg_runner' est déjà installé !" 2>/dev/null
    exit 0
  fi

  target_runner_dir="$lutris_runner_dir/$target_arg_runner"
  temp_dir=""
  source_type="${temp_source_type[$target_arg_runner]}"

  if [ "$source_type" = "[Local]" ]; then
    archive_path="${temp_file_or_url[$target_arg_runner]}"
  else
    temp_dir=$(mktemp -d)
    archive_path="$temp_dir/${target_arg_runner}.zgr"
    download_runner "${temp_file_or_url[$target_arg_runner]}" "$archive_path" "$target_arg_runner"
...
# ---------------------------------------------------------------------------------------------
# COLLECTE DES RUNNERS (Locaux + En ligne)
declare -A source_by_name
declare -A file_or_url_by_name

shopt -s nullglob
local_zgp_files=( "$search_dir"/*.zgr )
...
for runner_name in "${runners_to_install[@]}"; do
  # Si l'utilisateur choisit de parcourir un fichier externe
  if [ "$runner_name" = "Parcourir un fichier .zgr..." ]; then
    ...
    for ext_file in "$ext_dir"/*.zgr; do
      ...
      ext_source_by_name["$ext_r_name"]="[Local]"
